<template>
  <div class="container">
    ...
    <h1>Lab: System Calls [<a href="">GitHub Classroom Link</a>]</h1>
    ...
    <div class="prereq">
      <p>Before you start coding, read Chapter 2 of
        the <router-link to="/xv6-book">xv6 Book</router-link>, and
      ...
      <ul>
        ...
        <li> The user-space "stubs" that route system calls into the kernel are in <tt>user/usys.S</tt>,
          ...
          Declarations are in <tt>user/user.h</tt></li>

        <li> The kernel-space code that routes a system call to the
          kernel function that implements it
          is in <tt>kernel/syscall.c</tt> and <tt>kernel/syscall.h</tt>.</li>

        <li> Process-related code is <tt>kernel/proc.h</tt> and <tt>kernel/proc.c</tt>.</li>
    ...
    <ul>
      <li><router-link to="#gdb">Using gdb</router-link></li>
      <li><router-link to="#trace">System call tracing</router-link></li>
      <li><router-link to="#attack">Attack xv6</router-link></li>
    </ul>


    <p>If you run <kbd>make grade</kbd> you will see that the grading script cannot
      exec
    ...
    <h2 id="gdb">Using gdb (<span class="easy">easy</span>) </h2>
    ...
    <p>In many cases, print statements will be sufficient to debug your
      ...
      The GDB debugger can help.</p>

    <p> To help you become familiar with gdb, run <kbd>make qemu-gdb</kbd> and
      then fire up gdb in another window
      (see the gdb material on the <router-link to="/guidance">guidance page</router-link>). Once
      you have two windows open, type in the gdb window:</p>
    <pre>
...
(gdb) <kbd>backtrace</kbd>
    ...
    <p>Answer the following questions in <tt>answers-syscall.txt</tt>.</p>
    ...
    <p> Type <kbd>n</kbd> a few times to step past <tt> struct proc *p = myproc();</tt>
      ...
      (see <tt>kernel/proc.h></tt>) in hex.</p>
    ...
    <p>The processor is running in supervisor mode, and we can print privileged
      ...
      (see <a href="readings/priv-isa-asciidoc.pdf">RISC-V
        privileged instructions</a> for a description):</p>
...
panic: kerneltrap
    ...
    <p>To inspect the state of the processor and the kernel at the
      ...
      faulting <tt>epc</tt>, like this:</p>
    <pre>
...
Thread 3 hit Breakpoint 1, syscall () at kernel/syscall.c:138
    ...
    <div class="question">
      ...
      of <tt>scause</tt>
      in <a href="readings/priv-isa-asciidoc.pdf">RISC-V
    ...
    <p>You may want to revisit
      <a href="lec/gdb_slides.pdf">Using
        the GNU Debugger</a> as needed.
      The <router-link to="/guidance">guidance page</router-link> also has
      debugging tips.
    </p>

    <h2 id="trace">System call tracing (<span class="moderate">moderate</span>)</h2>
    ...
    <p>We provide a <tt>trace</tt> user-level program that runs another
      ...
      done, you should see output like this:</p>
...
$
    ...
    <ul>
      ...
      <li>
        <p>Add <tt>$U/_trace</tt> to UPROGS in Makefile</p>
      </li>

      <li>
        <p>Run <kbd>make qemu</kbd> and you will see that the
      ...
      <li>
        <p>Add a <tt>sys_trace()</tt> function
          ...
          in <tt>kernel/syscall.c</tt>.</p>
      </li>

      <li>
        <p>Modify <tt>fork()</tt> (see <tt>kernel/proc.c</tt>) to copy
          the trace mask from the parent to the child process. </p>
      </li>

      <li>
        <p>Modify the <tt>syscall()</tt> function
          in <tt>kernel/syscall.c</tt> to print the trace output. You will need to add an array of syscall names to
          index into.</p>
      </li>

    </ul>

    <h2 id="attack">Attack xv6 (<span class="moderate">moderate</span>)</h2>
    ...
    <p>The xv6 kernel isolates user programs from each other and isolates
      ...
      trick xv6 into revealing a secret from another process.</p>
    ...
    <p>The bug is that the call to <tt>memset(mem, 0, sz)</tt> at line 272
      ...
      contents from its previous use.</p>
    ...
    <div class="required">
      <p><tt>user/secret.c</tt> writes an 8-byte secret in its memory and
        ...
        different for each run of <tt>attacktest</tt>.)</p>
    ...
    <ul>
      ...
      <li> Run <tt>attacktest</tt> in the xv6 shell. It should the following output:
        ...
        <p>Note that despite the 3 deleted lines, xv6 appears to work correctly: it started the shell and it ran
          <tt>attacktest</tt>. In
      ...
      <li>Read <tt>user/attacktest.c</tt>. It generates a random 8-byte string,
        ...
        secret string to file descriptor 2.</li>

      <li>Read <tt>user/secret.c</tt> and think about how you could trick
        xv6 into revealing the secret to <tt>attack.c</tt>.</li>

      <li>Test your exploit by running <tt>attacktest</tt> in the xv6 shell.</li>
