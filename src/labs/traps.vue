...
<h1>Lab: Traps [<a href="">GitHub Classroom Link</a>]</h1>
...
<p>This lab explores how system calls are implemented using traps.
  ...
  implement an example of user-level trap handling.</p>

<div class="prereq">
<p>Before you start coding, read Chapter 4 of the <router-link to="/xv6-book">xv6 Book</router-link>, and
    related source files:</p>
  
  <ul>

    <li> <tt>kernel/trampoline.S</tt>: the assembly involved in
      changing from user space to kernel space and back</li>
      
    <li> <tt>kernel/trap.c</tt>: code handling all interrupts</li>
...
<ul>
  <li><router-link to="#assembly">RISC-V assembly</router-link></li>
  <li><router-link to="#backtrace">Backtrace</router-link></li>
  <li><router-link to="#alarm">Alarm</router-link></li>
</ul>

<h2 id="assembly">RISC-V assembly (<span class="easy">easy</span>) </h2>
...
<p>It will be important to understand a bit of
  ...
  <tt>user/call.asm</tt>.</p>

<p>Read the code in call.asm for the functions <tt>g</tt>, <tt>f</tt>, and <tt>main</tt>.
  The instruction
  manual for RISC-V is on the <router-link to="/reference">reference page</router-link>.
  Answer the following questions in <tt>answers-traps.txt</tt>:</p>
  ...
  <div class="question">
    At what address is the function <tt>printf</tt> located?
  </div>
  ...
  <h2 id="backtrace">Backtrace (<span class="moderate">moderate</span>) </h2>
 ...
 <p>For debugging it is often useful to have a backtrace: a list
  ...
  saved return address in each stack frame.</p>
...
<p>
  ...
  calls <tt>sys_sleep</tt>.</p>
  <p>Your output should be a list of return addresses with this form (but the numbers will likely be different):</p>
  <pre>
    backtrace:
    0x0000000080002cda
    0x0000000080002bb6
...
<p>Some hints:</p>
 ...
 <li>Add the prototype for your <tt>backtrace()</tt> to <tt>kernel/defs.h</tt> so that
  you can invoke <tt>backtrace</tt> in <tt>sys_sleep</tt>.</li>
...
r_fp()
{
 ...
 <li>These
 <a href="lec/l-riscv.txt">lecture
 ...
 stackframe, and that the saved frame pointer lives at fixed offset (-16) from the frame pointer.</li>
 ...
 page that a frame pointer refers to.</li>
 </ul>
...
<h2 id="alarm">Alarm (<span class="hard">hard</span>) </h2>
...
is correct if it passes alarmtest and 'usertests -q'</p>
</div>
...
stop generating periodic alarm calls.</p>
...
<p>When you're done, your solution will be only a few lines of code,
  ...
  says that all the tests pass.</p>
...
program crashes after printing "alarm!". Here are some hints:</p>

<ul>

<li>You'll need to modify the Makefile to cause <tt>alarmtest.c</tt>
to be compiled as an xv6 user program.</li>

<li>The right declarations to put in <tt>user/user.h</tt> are:</li>
<pre>
...
<li>Update user/usys.pl (which generates user/usys.S),
   ...
   sigreturn system calls.</li>

<li>For now, your <tt>sys_sigreturn</tt> should just return zero.</li>
...
structure (in <tt>kernel/proc.h</tt>).</li>
...
in <tt>usertrap()</tt> in <tt>kernel/trap.c</tt>.</li>


<li>You only want to manipulate a process's alarm ticks if there's a 
  timer interrupt; you want something like</li>
<pre>
    if(which_dev == 2) ...
</pre>
...
<li>Only invoke the alarm function if the process has a
 ...
 <li>You'll need to modify
  ...
  the instruction address at which user-space code resumes execution?</li>

<li>It will be easier to look at traps with gdb if you tell qemu to
use only one CPU, which you can do by running</li>
<pre>
    make CPUS=1 qemu-gdb
</pre>

<li>You've succeeded if alarmtest prints "alarm!".</li>
...
off, so that the handler is called periodically.</p>
...
process to resume properly after it has handled the alarm.</p>
...
<p>
  ...
  <ul>
    <li>Your solution will require you to save and restore
      ...
      the interrupted code correctly? (Hint: it will be many).</li>
    ...
    <li>Make sure to restore a0.  <tt>sigreturn</tt> is a system call,
      and its return value is stored in a0.</li>
